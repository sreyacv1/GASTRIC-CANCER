...
Median values and Cliff's delta for the non-ulcer-control versus cancer-adjacent-mucosa (GCN) contrast. The non-ulcer, ulcer and GCN groups share sequencing flowcells, so contrasts among them are not confounded by batch (unlike any contrast involving tumour tissue, whose libraries sit on separate flowcells). Negative delta indicates lower diversity in GCN.
...
## Supplementary Table S7 — Data-acquisition and reproducibility checklist
...
## Supplementary Table S8 — WGCNA soft-thresholding power sensitivity
